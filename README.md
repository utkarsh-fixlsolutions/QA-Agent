...
| [34](docs/34-g5-repair-and-completion-design.md) G5.3/G5.4: repair integration + completion semantics | `runtime_repair` becomes a real, eligible action wired to the existing, unmodified G4 pipeline - at most one attempt per failure, never reported as successful without a real re-verification; a new `compute_qa_outcome` reports a session's real `passed`/`failed`/`inconclusive` result from evidence alone |
| [36](docs/36-api-qa-report-export.md) API QA report export · [37](docs/37-api-qa-server-log-capture.md) server log capture · [38](docs/38-broader-route-discovery.md) broader route discovery · [39](docs/39-connect-probe.md) connect probe | The web UI (`web/`): a browser upload flow over the same existing engine, HTML/CSV report export, real dev-server log capture on failure, wider Next.js/Express route discovery, and a real TCP connect-probe fix for a ready-signal race |
| [40](docs/40-local-deployment.md) local deployment | Deploying `web/` on-premise (not Vercel/serverless - technically incompatible with its long-running dev-server subprocesses): a pinned `web/requirements.txt`, a `127.0.0.1`-only `web/run_service.ps1` launch wrapper, and the Windows Scheduled Task command to run it unattended at logon |
...
**G5.3 (Repair Integration) + G5.4 (Completion Semantics) complete - G5 (G5.1-G5.4) done:** `runtime_repair` (`qa_agent/agent/executor.py`), previously a registry entry deliberately left `implemented=False` since G5.1, now calls G4's real `repair_runtime_failure` unmodified for one diagnosed, currently-unrepaired target check - a check id that receives any repair result, success or failure, is never offered again this session (docs/24's own "at most one attempt per failure" rule, enforced structurally, no retry counter anywhere). A new `qa_agent/agent/completion.py` adds `compute_qa_outcome(state)` - a deterministic, evidence-only `passed`/`failed`/`inconclusive` verdict on `AgentResult.qa_outcome`, computed only from real execution/repair evidence, independent of why a session stopped or what the AI itself claimed; a planned check that never actually ran is `inconclusive`, never falsely `passed`. Verified end to end with a real, scripted provider driving the real loop through a real broken build, real diagnosis, real repair proposal, real temporary-workspace apply, real candidate re-check, real write, and real re-verification - the file on disk was actually patched. See [docs/34](docs/34-g5-repair-and-completion-design.md).

**Local deployment prepared (Scheduled Task registration still pending, needs the user's own action):** `web/requirements.txt` pinned to verified-installed versions; new `web/run_service.ps1` launches `uvicorn web.server:app` bound to `127.0.0.1:8000` only via the project's own `.venv` interpreter, logging to `web/logs/` (gitignored) - verified by hand to bind and serve a real `200`. Vercel/serverless hosting was ruled out, not just by the project's own on-premise-only constraint but technically: the app spawns long-running dev-server subprocesses (`--api-test`) that serverless platforms cannot host. Registering the actual Windows Scheduled Task (auto-start at logon, auto-restart on crash) was refused by the agent's own safety guard as a persistent system change requiring explicit user action - the exact command is in docs/40. See [docs/40](docs/40-local-deployment.md).
